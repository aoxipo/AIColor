...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        if not th.cuda.is_available():
            ...
            self.ddp_model = DDP(
                self.model,
                device_ids = [th.device(f"cuda")],
                output_device= th.device(f"cuda"),
    ...
    def run_loop(self):
        ...
        while (
            ...
            try:
                    ...
                    data_iter = iter(self.dataloader)
                    batch, cond, name = next(data_iter)
            # print("run loop:", batch.shape, cond.shape)
            ...
            if self.step % self.save_interval == 0:
                ...
                if os.environ.get("DIFFUSION_TRAINING_TEST", "") and self.step > 0:
                    return
                print("end of save interval here")
                # return 
            self.step += 1
            # print(self.step+"/"+self.save_interval)
            if self.step % len(self.dataloader) == 0:
                print( f"{self.step}/{self.save_interval} ETA{ (self.save_interval - self.step) * (datetime.datetime.now() - s).seconds/60 }" )
            if self.step % 200000 == 0:
                print( "train in 200,000 and down." )
                return 
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            # print("schedule_sampler:", weights.shape, t)
