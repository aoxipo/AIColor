...
from .GTU_model.GT_UNet import SKConv
# from .GTU_model.GT_UNet_D import ResGroupFormer_ddpm
...
from .GTU_model.GT_UNet import _make_bot_layer

class ResGroupFormer(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            normalization(channels),
            nn.SiLU(),
            _make_bot_layer(ch_in = channels, ch_out = self.out_channels),
            # conv_nd(dims, channels, self.out_channels, 3, padding=1),
        )
        ...
        self.out_layers = nn.Sequential(
            ...
            #     conv_nd(dims, self.out_channels, self.out_channels, 3, padding=1)
            # ),
            _make_bot_layer(ch_in = self.out_channels, ch_out = self.out_channels),
        )
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = nn.Sequential(
                conv_nd(dims, channels, self.out_channels, 3, padding=1),
                normalization8(self.out_channels),
                nn.SiLU(),
        ...
        self.sk = SKConv([self.out_channels, self.out_channels], self.out_channels, 32, 2,8,2 )
    ...
    def _forward(self, x, emb):
        ...
        if self.use_scale_shift_norm:
            ...
            h = self.out_layers(h)
        # return self.skip_connection(x) + h
        return self.sk( [ self.skip_connection(x), h])
...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        time_embed_dim = model_channels * 4
        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim), # 128, 128 * 4
            nn.SiLU(),
            linear(time_embed_dim, time_embed_dim),
        )
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []
        # print("time shape:", timesteps.shape)
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        # print("time/emb shape:", timesteps.shape, emb.shape)
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)
        ...
        h = x.type(self.dtype)
        c = h[:,:-1,...]
        # print("c:", c.shape, x.shape)
        anch, cal = self.highway_forward(c)
        # print("ext feature end:", anch[0].shape, anch[1].shape, cal.shape)
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                emb = emb.squeeze()
            if ind == 0: 
...
class UNetModel_rg(nn.Module):
    ...
    def __init__(
        ...
        self.sk = SKConv( [model_channels, model_channels], model_channels, 32, 2, 8, 2 )
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for ind, module in enumerate(self.input_blocks):
            ...
            if ind == 0: 
                h = module(h, emb)
                tp = th.cat((anch[0], anch[0], anch[1]),1).detach()
                # h = h + tp# 32 + 32 + 64 in 256 res
                h = self.sk([h, tp])
            else:
                h = module(h, emb)
            hs.append(h)
